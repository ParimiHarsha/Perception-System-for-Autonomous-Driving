...
class RoadSegmentation3D:
    ...
    def process_loop(self, event):
        ...
        left_boundary_3d = self.find_matching_points_kdtree(np.array(self.msgLeftBoundary.RoadArea.data).reshape(-1, 2), u, v, pc_arr)
        right_boundary_3d = self.find_matching_points_kdtree(np.array(self.msgRightBoundary.RoadArea.data).reshape(-1, 2), u, v, pc_arr)
