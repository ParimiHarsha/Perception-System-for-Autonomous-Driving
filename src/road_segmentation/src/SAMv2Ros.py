...
def process_image(image, publish_image=False, resize_dimensions=None):
    """
    Process the given image to detect road boundaries.

    Args:
        image (np.ndarray): The input image.
        publish_image (bool): Whether to publish the processed image. Defaults to False.
        resize_dimensions (tuple): Resize dimensions in the format (width, height). If None, use the original image size.

    Returns:
        overlay (np.ndarray): Overlay image with detected boundaries (if publish_image is True).
        left_boundary_points (np.ndarray): Detected left boundary points.
        right_boundary_points (np.ndarray): Detected right boundary points.
    """
    rospy.loginfo("Starting image processing function.")

    # Set the resize dimensions to the original image size if not provided
    h_orig, w_orig, _ = image.shape
    if resize_dimensions is None:
        resize_dimensions = (w_orig, h_orig)

    # Convert the image to a torch tensor and move it to the device
    start_step = time.time()
    image_resized = cv2.resize(image, resize_dimensions)
    ...
    # Calculate the center line on the Y-axis for boundary classification
    center_x = w_resized * 0.75
    ...
    MIN_Y_COORD = int(0.98 * h_resized)

    # Use the largest contour as the road area
    road_contour = max(contours, key=cv2.contourArea)
    if cv2.contourArea(road_contour) < MIN_CONTOUR_AREA:
    ...
    # Identify the horizon point (the point where left and right boundaries meet)
    horizon_index = np.argmin(contour_points[:, 1])
    horizon_point = contour_points[horizon_index]

    # # Classify left and right boundary points using horizontal bins
    # left_boundary_points, right_boundary_points = (
    #     classify_boundaries_using_horizontal_bins(contour_points, h_resized)
    # )
    # Classify left and right boundary points using horizontal bins
    left_boundary_points, right_boundary_points = (
        classify_boundaries_using_horizontal_bins(
            contour_points[contour_points[:, 1] < center_x], h_resized
        )
    )
    ...
    # Create a contour overlay image if publishing is enabled
    overlay = None
    if publish_image:
        contour_overlay = image_resized
        mask = (
            masks_tensor.sum(dim=0)
            .cpu()
            .numpy()
