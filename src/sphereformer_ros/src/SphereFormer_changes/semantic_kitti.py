...
import torch
import yaml
from SphereFormer_changes.data_util import data_prepare
...
def elastic(x, gran, mag):
    ...
    noise = [
        scipy.ndimage.filters.convolve(n, blur0, mode="constant", cval=0) for n in noise
    ]
    noise = [
        scipy.ndimage.filters.convolve(n, blur1, mode="constant", cval=0) for n in noise
    ]
    noise = [
        scipy.ndimage.filters.convolve(n, blur2, mode="constant", cval=0) for n in noise
    ]
    noise = [
        scipy.ndimage.filters.convolve(n, blur0, mode="constant", cval=0) for n in noise
    ]
...
class SemanticKITTI(torch.utils.data.Dataset):
    def __init__(
        ...
        split="train",
        return_ref=True,
        label_mapping="src/sphereformer_ros/src/semantic_kitti.yaml",
        rotate_aug=True,
        flip_aug=True,
        scale_aug=True,
    ...
    def get_single_sample(self, index, vote_idx=0):
        ...
        annotated_data = np.fromfile(
            file_path.replace("velodyne", "labels")[:-3] + "label", dtype=np.uint32
        ).reshape((-1, 1))
        ...
        if self.elastic_aug:
            points[:, 0:3] = elastic(
                points[:, 0:3], self.elastic_gran[0], self.elastic_mag[0]
            )
            points[:, 0:3] = elastic(
                points[:, 0:3], self.elastic_gran[1], self.elastic_mag[1]
            )
        ...
        if self.split == "train":
            coords, xyz, feats, labels = data_prepare(
                xyz,
                feats,
                labels_in,
                self.split,
